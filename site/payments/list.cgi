...
print <<"__END__";
        <td align=left valign=top><h1>Make Payments </h1>
        <p>Click on the Invoice number column number to make a payment.<br><br>
...
# Get Invoices to pay
my $sthInvoices=$dbh->prepare("SELECT Invoice.InvoiceNo, CompanyName, InvoiceDay, InvoiceMonth, InvoiceYear, sum(GrossAmtUKP*Quantity) As AmtDue From Invoice,InvoiceDetails,CustomerDetails WHERE Invoice.InvoiceNo=InvoiceDetails.InvoiceNo AND Invoice.CoID=CustomerDetails.CoID AND Invoice.void is null Group By InvoiceNo");
...
while ( my $refInvoices = $sthInvoices->fetchrow_hashref() )
{
	...
	{
		$flag = 1 if ( $Payments{$refInvoices->{InvoiceNo}} < $refInvoices->{AmtDue} );
	}
	else
	{
		$flag = 1;
