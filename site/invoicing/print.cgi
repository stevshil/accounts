...
my $sth;
if ( $numMonths == 0 ) {
	$sth = $dbh->prepare("SELECT InvoiceNo, Invoice.CoID, CompanyName, InvoiceDay, InvoiceMonth, InvoiceYear, PrintedDay, PrintedMonth, PrintedYear, Posted FROM Invoice,CustomerDetails WHERE Invoice.CoID=CustomerDetails.CoID AND Invoice.void is null AND date(concat(InvoiceYear,'-',InvoiceMonth,'-',DAY(NOW()))) >= date_sub(NOW(),INTERVAL 3 MONTH)");
} elsif ( $numMonths == -1 ) {
