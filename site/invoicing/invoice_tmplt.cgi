...
if ( $refCustomer->{InvoiceNo} == 0 )
{
        $sthCustomer->finish();
        $sthCustomer = $dbh->prepare("SELECT Invoice.InvoiceNo, Invoice.InvoiceDay, Invoice.InvoiceMonth, Invoice.InvoiceYear, CurrencyType, CurrencyRate, CustomerDetails.CompanyName, CustomerDetails.Address1, CustomerDetails.Address2, CustomerDetails.Address3, CustomerDetails.TownCity, CustomerDetails.County, CustomerDetails.Country, CustomerDetails.Postcode, CustomerDetails.PaymentTermsID FROM Invoice,CustomerDetails WHERE Invoice.CoID = CustomerDetails.CoID AND InvoiceNo=$InvoiceNo");
        $sthCustomer->execute();
        $refCustomer = $sthCustomer->fetchrow_hashref();
}
...
<tr><td align=right colspan=2>
	<table border=0><tr><td align=left>Invoice No:</td><td align=right><b>$refCustomer->{InvoiceNo}</b></td></tr>
...
<tr><td align=left colspan=2>$refCustomer->{CompanyName}</td></tr>
...
print "<tr><td align=left colspan=2>$refCustomer->{County} &nbsp; &nbsp; &nbsp; $refCustomer->{Postcode}</td></tr>\n" if ( "$refCustomer->{County}" ne "" );
...
{
	print "<tr><td colspan=2><table cellpadding=0 cellspacing=0>\n";
...
print <<"__END__";
<tr><td colspan=2 align=center>Please make cheque(s) payable to $COMPANYNAME at the address below.<br>Payment requested within $ref->{PaymentTerms} of Invoice Date.<br>Any payments received after this time are subject to $INTERESTRATE% interest per day unpaid.<br>Days includes weekends and bank holidays<br>
&nbsp;<br>
$COMPANYNAME<br>
$COMPANYADDRESS<br>
